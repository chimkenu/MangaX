    ...
    public DelawareSmash() {
        super((plugin, entity) -> {
            ...
            for (int i = 0; i < 7; i++) {
                ...
                for (int j = 0; j < 42; j++) {
                    ParticleEffects.create(plugin, loc.getWorld(), loc.toVector(), loc.getDirection(), 15, 15, new ParticleEffects.Effect() {
                        @Override
                        ...
                        public void intersect(LivingEntity livingEntity) {
                            if (livingEntity != entity && !livingEntity.getType().equals(EntityType.ARMOR_STAND) && livingEntity.getNoDamageTicks() <= 0) {
                                ...
                                MoveTargetEvent event = new MoveTargetEvent(Moves.DEKU_DELAWARE_SMASH, entity, livingEntity, 5, v);
