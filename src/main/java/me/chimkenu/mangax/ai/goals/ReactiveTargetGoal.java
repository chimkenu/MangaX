...
public class ReactiveTargetGoal extends TargetGoal {
    ...
    public Action generateNextAction() {
        ...
        moveMap.forEach((moves, cooldown) -> {
            ...
            if (cooldown <= 0) actions.add(new Action(this, info.chargeTime() + (info.type() == MoveInfo.Type.CONTROL || info.type() == MoveInfo.Type.DEBUFF ? 0 : info.duration()), new RushStrafe(20, 1), null) {
                @Override
                public void start() {
                    super.start();
